...
# -- Utility Functions --------------------------------------------------------
# ==============================================================================
save_interval = 1  # Save data every 'save_interval' frames
...
class Env_Manager:
    ...
    def generate_route(self):
        """
        基于当前玩家的位置生成一个预定义的路径作为全局规划。
        """
        carla_map = self.world.get_map()
        # 1. 获取当前玩家车辆的位置作为起点
        player_location = self.player.get_location()
        # 获取该位置最近的道路航点
        start_wp = carla_map.get_waypoint(player_location)

        global_plan = []
        current_wp = start_wp
        target_distance = 200.0  # 目标总行驶距离，可根据你的场景需要调整
        accumulated_distance = 0.0

        # 2. 从起点开始，向前生成一系列航点，直到达到目标距离
        while accumulated_distance < target_distance:
            global_plan.append(current_wp)
            # 每次向前取5米的下一个航点
            next_wps = current_wp.next(5.0)
            if next_wps:
                # 这里选择第一个（最直）的后续航点。对于复杂路口，你可能需要根据道路选项（转弯指令）来选择
                next_wp = next_wps[0]
                # 计算累计距离
                accumulated_distance += current_wp.transform.location.distance(next_wp.transform.location)
                current_wp = next_wp
            else:
                # 如果没有后续航点了（例如到达道路尽头），跳出循环
...
def game_loop(args):
    ...
    try:
        ...
        if player:
            if args.agent == "Basic":
                agent = BasicAgent(player, 30)
                agent.follow_speed_limits(True)
            elif args.agent == "Constant":
                agent = ConstantVelocityAgent(player, 30)
                ground_loc = world.world.ground_projection(player.get_location(), 5)
                if ground_loc:
                    player.set_location(ground_loc.location + carla.Location(z=0.01))
                agent.follow_speed_limits(True)
            ...
            destination = load_destination_from_json(args.destination_path)
            agent.set_destination(destination)

        print(f"DEBUG: Set autopilot for vehicle ID: {vehicle.id}, Role: {vehicle.attributes['role_name']}")
        ...
        while True:
            ...
            if results:
                ...
                # Print autopilot and vehicle information
                if player is not None:
                    ...
                    print(f"  Vehicle ID: {player.id}")
                    print(f"  Location: x={player.get_location().x:.2f}, y={player.get_location().y:.2f}, z={player.get_location().z:.2f}")
                    print(f"  Speed: {speed:.2f} km/h")
                    print(f"  Autopilot enabled: {player.is_alive and hasattr(player, 'get_autopilot')}")
                    print(f"  Control - Throttle: {control.throttle:.2f}, Steering: {control.steer:.2f}, Brake: {control.brake:.2f}")
                    
                    # Get current waypoint
                    current_waypoint = sim_world.get_map().get_waypoint(player.get_location())
                    print(f"  Current Waypoint: RoadID={current_waypoint.road_id}, LaneID={current_waypoint.lane_id}")
                    print(f"  Waypoint Position: x={current_waypoint.transform.location.x:.2f}, y={current_waypoint.transform.location.y:.2f}")
        ...
        if sim_world is not None:
            ...
            sim_world.apply_settings(settings)
            print("World settings restored to asynchronous mode")
        # if player is not None:
        #     player.destroy()
...
def main():
    ...
    argparser.add_argument(
        '-d', '--destination_path',
        help='Set the json file for the destination point for the vehicle',
        default=None,)
